...
export function AdminProducts() {
  ...
  const productsQuery = useQuery({
    queryKey: ["products"],
    queryFn: () => apiFetch<PaginatedResponse<Product>>("/api/products")
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      if (!editProduct) throw new Error("Produto inválido");
      ...
      if (editImageFile) {
        try {
          await apiUpload<Product>(`/api/products/${editProduct.id}/upload-image`, editImageFile);
        } catch (error) {
          console.error("Erro ao fazer upload da imagem:", error);
          // Não falha a atualização se upload falhar
        }
      }
  ...
  const cards = productsQuery.data?.data ?? [];
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6">
        {cards.map((product) => (
          <div key={product.id} className="rounded-2xl border border-brand-100 bg-white p-3">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-sm font-semibold leading-tight">{product.name}</p>
                <p className="text-xs text-slate-500">{formatCurrencyValue(parseNumericField(product.price))}</p>
              </div>
            </div>
            {product.imageUrl ? (
              <img
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setEditPrice(formatCurrencyValue(parseNumericField(product.price)));
                  setEditImageFile(null);
        ...
        <Modal title="Detalhes do produto" onClose={() => setViewProduct(null)}>
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">{formatCurrencyValue(parseNumericField(viewProduct.price))}</p>
            </div>
            {viewProduct.description ? (
              <p className="text-sm text-slate-600">{viewProduct.description}</p>
            ) : null}
      ...
      {editProduct ? (
        <Modal title="Editar produto" onClose={() => setEditProduct(null)}>
          <div className="space-y-3">
            <input
              value={editName}
            ...
            <input
              value={editPrice}
              onChange={(event) => setEditPrice(formatCurrencyInput(event.target.value))}
              placeholder="Preço *"
              className="w-full rounded-xl border border-slate-200 px-3 py-2"
            />
            <textarea
